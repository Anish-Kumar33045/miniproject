...
df = results.copy()
if 'is_fraud_predicted' not in df.columns:
    df['is_fraud_predicted'] = 0
...
if 'hour' in df.columns and 'day_of_week' in df.columns:
    ...
    heat_pivot.index = [day_labels[i] for i in heat_pivot.index if i < len(day_labels)]
...
with col1:
    ...
    if 'merchant_cat' in df.columns:
        ...
        fig2 = go.Figure(go.Bar(
            x=merch['fraud_rate'], y=merch['merchant_cat'], orientation='h',
...
c3.metric("Top merchant", df['merchant_cat'].mode()[0] if 'merchant_cat' in df.columns and len(df) > 0 else "N/A")
c4.metric("Fraud rate",   f"{df['is_fraud_predicted'].mean()*100:.1f}%")

if 'bank' in df.columns:
    st.markdown("#### Transactions by bank")
    bank_df = df.groupby('bank')['amount'].count().reset_index()
    bank_df.columns = ['bank','count']
    fig4 = px.pie(bank_df, names='bank', values='count', hole=0.4, height=280)
